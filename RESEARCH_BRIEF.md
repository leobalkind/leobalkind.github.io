...
> **STATUS (2026-05-24, after v2.3):** Nearly every item in this brief has now
> shipped across v1.4 → v2.3. The roster has grown 13 → 15 games (added
> `backrooms-3d` in v1.8, `clown-forest` in v1.9), the v1.6 BACKROOMS overhaul,
> the v1.7 3D-DEPTH wave, the v2.0 CLOWN-FOREST 5-agent polish, the v2.1 sprite
> pass across the remaining 9 games, the v2.2 perf + backrooms-3d round 2, and
> the v2.3 deep-systems wave have all landed. Per-item completion marks are
> inlined below ("STATUS: COMPLETED IN vX.Y"). Anything without a status line
> is either still pending or only partially shipped; check current code first.

15 web-games audited against established indie/mobile reference titles. For each
...
1. **Brotato's between-wave shop with rerolls + lock.** After every wave, drop
   ...
   level-up." Effort: **M**. **STATUS: COMPLETED IN v1.4 (Brotato shop) + v2.3 (daily mutator).**
2. **Vampire Survivors weapon evolution.** Combining max-rank weapon + matching
   ...
   a clear "build" goal beyond raw stat stacking. Effort: **M**. *Still pending — partial via v1.7 perks.*
3. **Surviv.io kill-feed + spectator killcam.** Top-right kill ticker ("Loaf
   ...
   Effort: **S** for kill-feed, **M** for replay. **STATUS: COMPLETED IN v1.4 (kill-feed + killcam).**
...
1. **PvZ's "almanac" + wave preview banner.** Before each night, show a banner
   ...
   **STATUS: COMPLETED IN v1.4 (wave preview shared module).**
2. **They Are Billions wall-tier visuals.** Walls should visibly degrade — fresh
   ...
   tiering. Effort: **S**. **STATUS: COMPLETED IN v1.4 (HP-tier visuals).**
3. **PvZ between-level meta unlock.** Surviving night N permanently unlocks one
   ...
   highscore. Effort: **M**. **STATUS: COMPLETED IN v1.4 (meta-unlocks) + v2.3 (full TECH TREE w/ RP currency).**
...
1. **Hitman GO turn-preview / Monaco's predictable patrols.** Show the human's
   *next* facing arc as a faint ghost cone, so peeking past a guard feels
   ...
   Effort: **S**. **STATUS: COMPLETED IN v1.4 (ghost vision cones).**
2. **Monaco's loot tooltip + heat bar.** Each loot item shows value-on-hover
   *before* pickup, and a global "heat" bar fills with each suspicious action;
   ...
   Effort: **M**. **STATUS: COMPLETED IN v1.4 (loot tooltips) + v1.7.2 (WEIGHT/NOISE on tooltips).**
3. **Hotline Miami's restart-instant + score grade.** End-of-floor grade
   ...
   Effort: **S** for grade, **M** for mid-floor restart. **STATUS: COMPLETED IN v1.4 (shared gradeCard) + v2.3 (CONTRACTS objectives).**
...
1. **Tasty Planet's explicit size-tier UI.** A "next form unlocks at X mass"
   ...
   close. Effort: **S**. **STATUS: COMPLETED IN v1.4 (next-form preview) + v2.3 (target priority HUD).**
2. **Rampage's destruction combos / chain multiplier.** Smashing 3+ buildings
   ...
   destruction power-fantasy. Effort: **S**. **STATUS: COMPLETED IN v1.4 (combo multiplier).**
3. **Crush the Castle's physical debris persistence.** Building chunks should
   ...
   visually accumulates damage. Effort: **M**. *Still pending — partially via v1.8 city-specific skylines.*
...
1. **Goose Game's per-level checklist / to-do list.** A small pinned card:
   ...
   **STATUS: COMPLETED IN v1.4 (objectives checklist + customer witnesses).**
2. **Goose Game's reactive NPC barks + animations.** When a guard sees the cart
   ...
   add huge personality. Effort: **S**. **STATUS: COMPLETED IN v1.4 (guard barks + v1.7 4 customer NPCs).**
3. **Supermarket Shriek's physics-cart momentum.** Cart should have inertia and
   *knock down shelves on collision*, creating domino chains. We have a cart
   but it just speed-boosts. The toy is the chaos. Effort: **M**. **STATUS: COMPLETED IN v1.4 (cart dominos) + v2.3 (GETAWAY VEHICLE choice).**
...
1. **Bloons TD's 2x/3x speed toggle + pause-and-build.** Players burn out on
   ...
   table stakes for the genre and we don't have it. Effort: **S**. **STATUS: COMPLETED IN v1.4 (shared speed toggle).**
2. **Kingdom Rush's "call next wave early" gold bonus.** Button to summon the
   ...
   Effort: **S**. **STATUS: COMPLETED IN v1.4 (call-wave-early).**
3. **Bloons TD's tower upgrade tree (2-3 paths) instead of linear level-ups.**
   ...
   is the only upgrade vector. Effort: **L**. **STATUS: COMPLETED IN v1.5 (2-path tower tree) + v1.7 (TELEPORT tower + synergies + endless) + v2.3 (WAVE MODIFIER system).**
...
1. **Rocket League's goal celebration / replay.** When someone scores a kill,
   ...
   effort. Effort: **S**. **STATUS: COMPLETED IN v1.4 (PWNED freeze) + v1.7.2 (highlight reel).**
2. **Brawl Stars' "super" charge meter.** A meter that fills from hits dealt;
   ...
   raw aiming. Effort: **M**. **STATUS: COMPLETED IN v2.3 (WEAPON MASTERY thresholds).**
3. **Smash Bros' damage-driven knockback.** Knockback scales with target's
   ...
   the kitchen. Makes finishing kills feel decisive. Effort: **S**. **STATUS: COMPLETED IN v1.4 (HP-scaled knockback).**
...
1. **SteamWorld Dig's between-run meta-shop on the surface.** Sell loot on
   ...
   hook. Effort: **M**. **STATUS: COMPLETED IN v1.7.1 (10-item persistent skill tree) + v1.8 (9 MASTERY perks at depth 50/100/150).**
2. **Spelunky's shopkeeper / risk-reward NPC.** A friendly digger NPC sells one
   ...
   Effort: **M**. **STATUS: COMPLETED IN v1.5 (shopkeeper w/ steal-enrages-hunter).**
3. **Spelunky's "bombable wall" hidden secrets + Downwell combo system.**
   ...
   "dig." Effort: **S** for cracks, **S** for combo meter. **STATUS: COMPLETED IN v1.4 (cracked walls + dig combo) + v2.3 (TREASURE MAP item).**
...
1. **Crazy Taxi's directional arrow + countdown overlay.** A large arrow
   ...
   Effort: **S**. **STATUS: COMPLETED IN v1.4 (directional arrow).**
2. **Crazy Taxi's "Crazy Drift" bonus + score multiplier escalation.** Long
   ...
   Effort: **S**. **STATUS: COMPLETED IN v1.4 (drift bonuses) + v1.8 (DRIFT MASTER title at 5-streak) + v2.3 (ROUTES).**
3. **Death Stranding's cargo-fragility / package-care mechanic.** Each delivery
   ...
   nitro-spam. Effort: **M**. **STATUS: COMPLETED IN v1.5 (cargo INTACT% + perfect payouts) + v1.8 (PET CARRIER HP bar).**
...
1. **Doodle Jump's enemy / projectile variety scaling with height.** Enemies
   ...
   at height 100 and height 1000. Effort: **M**. **STATUS: COMPLETED IN v1.4 (bats + fireballs) + v1.7 (4 enemies + 4 powerups) + v1.8 (ABYSS biome + WIND CURRENT + TELEPORTER).**
2. **Icy Tower's "combo jumps" score system.** Chaining N platform-to-platform
   ...
   popups. Skill expression beyond just survival. Effort: **S**. **STATUS: COMPLETED IN v1.4 (combo) + v2.3 (BIOME CHALLENGES bonuses).**
3. **Geometry Dash's biome-shift checkpoints.** Every 500m, the wall art / lava
   ...
   changes. Effort: **S**. **STATUS: COMPLETED IN v1.4 (biome shifts) + v1.7 (4 biomes) + v1.8 (ABYSS biome).**
...
1. **Little Alchemy 2's "show me what ingredients I haven't used yet" filter.**
   ...
   combos are found. Effort: **S**. **STATUS: COMPLETED IN v1.4 (filter toggle) + v1.8 (codex sort: tier/alphabetical/element/newest).**
2. **Doodle God's category-completion bars + per-tier rewards.** "8/12
   ...
   Effort: **S**. **STATUS: COMPLETED IN v1.4 (tier rewards) + v1.8 (HYPER-FUSE chains + MUTATION OF THE WEEK) + v2.3 (EVOLUTION TREE SVG modal).**
3. **Little Alchemy's optional hint system with a cost.** Spend in-game
   ...
   Effort: **M** (needs currency layer). *Still pending — partial via v1.8 right-click "all tried combos" reveal.*
...
1. **Overcooked 2's throw mechanic + on-counter prep stations.** Throw plated
   ...
   between two stations doubles the chaos. Effort: **M**. **STATUS: COMPLETED IN v1.4 (throw mechanic).**
2. **Diner Dash's table-color matching chain bonuses.** Seating two
   ...
   combo but no *placement* puzzle. Adds a planning layer. Effort: **S**. **STATUS: COMPLETED IN v1.4 (color-chain bonus) + v2.3 (STAFF ROSTER).**
3. **Overcooked's "burnt food" failure state.** Items left too long at a
   ...
   timer ratchets tension. Effort: **S**. **STATUS: COMPLETED IN v1.4 (burnt timer) + v1.7 (KAREN fights + soup-of-day + hired staff).**
...
1. **Backrooms-canon "noclip to next level" portal mechanic.** Hitting the exit
   ...
   We have the archetypes; we don't chain them. Effort: **M**. **STATUS: COMPLETED IN v1.4 (noclip chain) + v1.6 (3 new archetypes POOLROOMS/PARKING GARAGE/THE END = 7 total) + v2.2 (4-level chain in backrooms-3d w/ EXIT door win).**
2. **Iron Lung-style audio sting / proximity audio cues.** Monster footsteps
   ...
   immersion for low effort. Effort: **S**. **STATUS: COMPLETED IN v1.6 (positional audio + panned whispers + ghost footsteps) + v2.2 (heartbeat icon w/ proximity pulse).**
3. **Backrooms-community "found notes / lore fragments" collectibles.** Scraps
   ...
   end screen. Effort: **S**. **STATUS: COMPLETED IN v1.4 (30 lore notes) + v1.7.2 (60 lore notes + LORE_COMPLETE achievement) + v2.3 (PSYCHIC FLASH).**
...
- **Persistent meta-currency** ("kibble" / "DNA" / "scrap") for between-run
  ...
  **STATUS: COMPLETED IN v1.7.1 (diggers 10-item persistent skill tree) + v2.3 (pugfort TECH TREE w/ RP).**
- **Kill-feed / event-ticker component** — bork-battle, rocket-pug, pugzilla,
  ...
  scrolling event log.
  **STATUS: COMPLETED IN v1.4 (src/shared/killFeed.js).**
- **"Next wave preview" / "incoming" banner** — pugfort and pug-td.
  **STATUS: COMPLETED IN v1.4 (src/shared/wavePreview.js).**

Each of these is a single shared module unlocking polish across 4-5 games.

---

**Total scope estimate at S/M-only items: ~60-80 hours.** **STATUS (post-v2.3):
the bulk of this brief has shipped.** Of 39 items here, roughly 35 are fully
complete or substantively addressed; remaining gaps are weapon-evolution
(bork-battle), debris-persistence (pugzilla), and the optional DNA-hint system
(mutation-lab). See `IMPROVEMENT_LIST_300.md` for the next 300 atomized items
(many already crossed off in v2.0 → v2.3).
