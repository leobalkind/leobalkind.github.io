...
document.getElementById('end-share')?.addEventListener('click', async () => {
  ...
  const text = `🐶 BORK BATTLE — I got ${kills} kills as ${form} in ${time}! Beat me at https://leobalkind.github.io/games/bork-battle/`;
  const btn = document.getElementById('end-share');
  try {
    if (navigator.share) {
      await navigator.share({ title: 'BORK BATTLE', text, url: 'https://leobalkind.github.io/games/bork-battle/' });
    } else {
