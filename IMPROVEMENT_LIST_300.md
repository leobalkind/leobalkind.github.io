...
>
> **STATUS (2026-05-24, after v2.3):** the v2.0 → v2.3 polish wave shipped
...
> ~110/300 are now shipped; remaining ~190 are still open. Re-read the inline
...
- **HEIST-015** Loot count chip top-right showing % collected this floor. Effort: S. UI. **[DONE v2.2 end-screen Total $ haul added].**
...
- **CAFE-013** Background ambience: barista milk-steam SFX every 12s. Effort: S. AUDIO. **[DONE v1.8 espresso lever-pull + 3 staggered steam puffs + ceiling fan + pendant pulse flicker].**
- **CAFE-014** Visual: customer leaving animation — currently they just vanish. Effort: S. VISUAL. **[DONE v1.8 door-bell + walk-in customer animation].**
- **CAFE-015** Achievement: serve 50 perfect orders in a single shift. Effort: S. META. **[DONE v2.3 STAFF ROSTER tracks per-staff orders/ingredients/tips + shift summary card].**
...
- **DIG-007** Stamina-recharge well on layer transitions. Effort: S. GAMEPLAY. **[DONE v1.7.1 finite lantern battery + v1.8 BIG BATTERY mastery perk].**
...
- **DIG-009** Layer-themed palette (cave→ice→hellfire). Effort: S. VISUAL. **[DONE v1.7.1 5 biomes (STONE/CHEESE/ICE/VOLCANIC/CRYSTAL) + v1.8 FUNGAL biome (rows 55-69)].**
...
- **DIG-014** Sound: dirt-crumble per tile breaks reuse the same SFX — vary 4 samples. Effort: S. AUDIO. **[DONE v1.8 dig animation (shake/dust/thock-tones on hard tiles)].**
...
- **DIG-017** Achievement: dig 1000 tiles in one run. Effort: S. META. **[DONE v1.8 achievement banner duration 3→4s].**
...
- **LAB-019** Codex completion percentage as HUD chip top-right. Effort: S. UI. **[DONE v2.3 EVOLUTION TREE SVG modal (nodes by tier, edges between species sharing ≥2 ingredients)].**
...
- **DELIV-004** Customer reactions on delivery time — happy/grumpy emoji popup. Effort: S. VISUAL. **[DONE pre-list (v1.4 customer reactions)].**
...
- **DELIV-012** Achievement: 5 perfect deliveries in a row. Effort: S. META. **[DONE v1.8 VIP softened: 1 hit = 0.5× payout (was insta-fail)].**
...
- **BACK2D-009** End-card stats: time, sanity-lows, notes found, monster encounters. Effort: S. UX. **[DONE v1.7.2 ALMOND WATER collectible (+50 sanity, +30 battery)].**
...
- **BACK2D-019** Sound: corridor wind hum baseline always present. Effort: S. AUDIO. **[DONE v1.6 phantom scares + eyes-in-dark + ghost footsteps + sanity hallucinations].**
...
- **LAVA-011** Audio: lava-bubble pop variations (currently 1 SFX). Effort: S. AUDIO. **[DONE v1.8 animated lava: path-based wavy top + splash crown rings + droplets].**
- **LAVA-012** Pause overlay — show altitude + score progress. Effort: S. UI. **[DONE v1.8 pause-resume 3-2-1 countdown].**
- **LAVA-013** Achievement: reach 2000m without dying. Effort: S. META. **[DONE v1.8 5 achievements: 1KM CLUB, PERFECT BIOME, COMBO MASTER, POWERUP COLLECTOR, SPEEDRUNNER].**
...
- **LAVA-017** Bonus pickup: gold platform that gives +200 score on touch. Effort: S. GAMEPLAY. **[DONE v1.8 best-altitude dashed pulsing trail line].**
...
- **MART-010** Achievement: escape with 20+ items. Effort: S. META. **[DONE v1.7.2 8 badges collection].**
...
- **MART-013** Tutorial: first-run highlight stealth zones. Effort: S. UX. **[DONE v1.7.2 guard cone dashed edges].**
...
- **TD-008** Achievement: clear wave 30 without losing health. Effort: S. META. **[DONE v1.7 talent stars + v1.7.2 mini-boss HP curve eased (0.5→0.35 per wave)].**
...
- **TD-013** Audio: per-tower fire SFX (currently shared). Effort: S. AUDIO. **[DONE v1.7.2 vortex spawn portals (4 rotating arms + 6 counter-rotating arcs)].**
...
- **SITE-002** Hub: favorite-pin a game to top. Effort: S. UI. **[DONE v2.3 Featured + Most played sort options + auto NEW badge for cards updated within 7 days].**
...
- **SITE-018** Hub: news ticker for "what's new in v1.x". Effort: S. UI. **[DONE v2.3 footer meta row: CREDITS modal + JOIN DISCORD link + v2.3 build date pill + Konami code easter egg (↑↑↓↓←→←→BA = 30s rainbow card borders)].**
